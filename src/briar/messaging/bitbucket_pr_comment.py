...
class BitbucketPrCommentWriter(MessageWriter):
    ...
    def _cloud(self):
        if self._client is None:
            ...
            if self._username == "x-token-auth":
                self._client = Cloud(url=self.BASE, token=self._app_password, timeout=20)
            else:
                self._client = Cloud(url=self.BASE, username=self._username, password=self._app_password, timeout=20)
        return self._client
    ...
    def send(self, *, target: str, body: str, **extras: Any) -> SendResult:
        repo_addr, number = parse_pr_target(target, extras)
