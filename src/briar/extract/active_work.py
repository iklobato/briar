...
class ExtractActiveWork(KnowledgeExtractor):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            "--active-repo", action="append", default=[],
    ...
    def extract(self, args: argparse.Namespace) -> Optional[ExtractedSection]:
        ...
        return ExtractedSection(
            ...
            body=(
                "Live snapshot of what's in flight. Agents must avoid "
                "touching files referenced in open PRs to prevent merge "
                "conflicts."
            ),
    ...
    def _scan_repo(
        ...
        lines = [
            f"- #{r['number']} {r['title']!r:80}  by={r['user']}"
            + ("  [draft]" if r["draft"] else "")
            + f"  comments={r['review_comments']}"
            for r in pr_rows
        ]
        return ExtractedSection(
