...
class ArchetypePrFixer(AgentArchetype):
    ...
    backstory_template = (
        ...
        "GitHub identity, never a bot account. Set "
        "`git config user.name '{user.login}'` and "
        "`git config user.email '{user.email}'` (or a `<id>+{user.login}"
        "@users.noreply.github.com` no-reply equivalent) on the working "
        "tree before the first commit. NEVER commit as `github-actions"
        "[bot]`, `briar-bot`, `claude[bot]`, or any other bot identity. "
        "If you cannot resolve the human author's identity, STOP and "
        "surface the missing config rather than silently committing under "
        "a bot.\n"
        "\n"
        "NEVER:\n"
